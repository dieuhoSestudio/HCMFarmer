...
const userSetInLocalStorage = (user:any) => {
...
//============ Save User info in Local Storage =============*
// const userSetInCookie = (user:any) => {
//     cookieService.updateCookie('user', JSON.stringify(user));
// }
// // ============ Gets the logged in user from Cookie =============*
...
const postRegister = (url:string, data:any) => {
    return axios.post(url, data).then(response => {
...
const postForgetPwd = (url:string, data:any) => {
    return axios.post(url, data).then(response => {
        if (response.status === 400 || response.status === 500)
            throw response.data;
        return response.data;
